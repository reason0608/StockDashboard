...
test('確認股數與入帳、重載仍保留，現在庫存不會改寫歷史', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '設定 2330 2026-06-11 入帳狀態' }).click();
...
test('非法備份不寫入，合法備份需確認且可匯出', async ({ page }) => {
  ...
  const backup = { version: 1, confirmations: { [fixtureEvent.id]: { event: fixtureEvent, shares: 0, confirmedAt: '2026-06-10', received: false } } };
