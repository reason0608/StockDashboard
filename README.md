...
- 股息表顯示除息日、發放日、每股金額、已確認股數、稅前預估與入帳核對狀態。
- 年度與月份依發放日統計；未公告發放日暫歸除息年度，不歸入任何月份。
- 股數須手動確認，可主動帶入現在庫存。未確認不算成零元，不自動推定歷史資格。
- 每次配息保存股數快照，賣出持股後紀錄仍保留。Demo 與個人模式分開保存。
- 已確認入帳保留當時金額；公告修正時提示重新核對。僅標記股息追蹤，不自動新增現金流水，避免重複入帳。
...
每股金額 × 確認股數為**稅前預估**，顯示至小數點後兩位。不計算所得稅、補充保費、匯費或券商捨入規則，不預測未公告配息。股票股利及非台幣配息不在本版範圍內。
